...
defmodule Field do
  ...
  def search(field, {fx, fy} = from, {tx, ty} = to) do
    ...
    0
    |> time(fields, from, {tx, ty - 1})
    |> IO.inspect(label: "To the end")
    |> time(fields, {tx, ty}, {fx, fy + 1})
    |> IO.inspect(label: "Back to start")
    |> time(fields, from, {tx, ty - 1})
  end

  def time(since, fields, from, to) do
    try do
      bfs(fields, [{from, since}], to, MapSet.new())
    catch
      msg when is_bitstring(msg)-> raise(msg)
      minute when is_integer(minute) -> minute + 1
    end
  end
