...
defmodule BP do
  ...
  def max(bp, :ore), do: [bp.ore_robo, bp.clay_robo, elem(bp.obs_robo, 0), elem(bp.geode_robo, 0)] |> Enum.max()
  def max(bp, :clay), do: elem(bp.obs_robo, 1)
  def max(bp, :obsidian), do: elem(bp.geode_robo, 1)
  def max(bp, :geode), do: 100500
end
...
defmodule GameState do
  ...
  def add_robo(%GameState{robots: robots} = state, type) do
    %GameState{ state |
      robots: Map.update!(robots, type, fn v -> v + 1 end)
    }
  end
end
