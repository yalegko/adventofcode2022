...
defmodule Field do
  ...
  def search_with_elek(field, distances, cur, unvisited, time_left) do
    unvisited
    ...
    |> Enum.map(fn to ->
      ...
      field.valves[to].rate * time_after_open +
        search_with_elek(
          ...
          distances,
          to,
          unvisited |> Enum.reject(&(&1 == to)),
          time_after_open
        )
    end)

    # Or the elek starts to help you since that point.
    |> Enum.concat([
      search(field, distances, "AA", unvisited, 26)
    ])
    |> Enum.max()
  end
...
field
|> Field.search_with_elek(distances, "AA", worth_visiting, 26)
|> IO.inspect()
